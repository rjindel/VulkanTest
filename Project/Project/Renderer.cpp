...
bool Renderer::EnumerateDevices()
{
	...
	//Get queue data
	uint32_t queueCount;
	vkGetPhysicalDeviceQueueFamilyProperties(gpus[0], &queueCount, nullptr);

	Debug::Log(std::string("Using default device: ") + properties.deviceName + "Which has " + ToString(queueCount) + " queues");
	...
	vkGetPhysicalDeviceQueueFamilyProperties(gpus[0], &queueCount, queueProperties);
	...
	err = vkCreateDevice(gpus[0], &deviceCreateInfo, nullptr, &devices[0]);
...
bool Renderer::CreateSwapchain()
{
	...
	surfaceInfo.pNext		= wnd;

	auto err = vkCreateWin32SurfaceKHR(instance, &surfaceInfo, nullptr, &surface);

	//Get surface format
	uint32_t formatCount;
	err = vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[0], surface, &formatCount, nullptr);
	...
	err = vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[0], surface, &formatCount, surfaceFormats);
...
//	swapchainCreateInfo.presentMode = swapchainPresentMode;
	...
	err = vkCreateSwapchainKHR(devices[0], &swapchainCreateInfo, nullptr, &swapchain);
	if (err != VK_SUCCESS)
	{
		//TODO output an error 
		return false;
...
bool Renderer::CreateTri()
{
	...
	auto err = vkCreateBuffer(devices[0], &vbInfo, nullptr, &vertexBuffer);
	if (err != VK_SUCCESS)
	{
		//TODO output an error 
		return false;
	...
	vkGetBufferMemoryRequirements(devices[0], vertexBuffer, &memRequirements);
